    ...
    (describe "content-hash"
      (fn []
        (it "normalizes line endings"
          (fn []
            (let [hash1 (hash.content-hash "line1\nline2")
                  hash2 (hash.content-hash "line1\r\nline2")]
              (assert.equals hash1 hash2))))
        ...
        (it "trims leading and trailing whitespace"
          (fn []
            (let [hash1 (hash.content-hash "content")
                  hash2 (hash.content-hash "  \n  content  \n  ")]
              (assert.equals hash1 hash2))))

        (it "returns valid hash"
          (fn []
            (let [result (hash.content-hash "Test content")]
              (assert.is_valid_hash result))))))
        ...
        (it "returns false when content matches hash"
          (fn []
            (let [content "Test content"
                  stored-hash (hash.content-hash content)]
              (assert.is_false (hash.has-changed stored-hash content)))))

        (it "returns true when content differs from hash"
          (fn []
            (let [old-content "Original content"
                  new-content "Modified content"
                  stored-hash (hash.content-hash old-content)]
              (assert.is_true (hash.has-changed stored-hash new-content)))))
        ...
        (it "handles whitespace normalization in comparison"
          (fn []
            (let [content "Test content"
              ...
              (assert.is_false (hash.has-changed stored-hash content-with-whitespace)))))))))
